#!/usr/bin/env bash
#
# The command-line front door to focus.sh. With no verb it picks a task and
# starts working on it, which is the common case; anything else is handed
# straight through.
#
# Selection lives here rather than in focus.sh, which takes a key and never
# asks a question.
#
# Usage:
...
A verb is handed to focus.sh unchanged.
USAGE
}

case "${1:-}" in
    -h|--help)         usage; exit 0 ;;
    start|stop|status) exec "$SCRIPT_DIR/focus.sh" "$@" ;;
    ""|-*)             ;;  # nothing but flags: pick something
    *)                 exec "$SCRIPT_DIR/focus.sh" start "$@" ;;
esac
...
exec "$SCRIPT_DIR/focus.sh" start "$key" "$@"
