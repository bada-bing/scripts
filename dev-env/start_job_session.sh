#!/bin/bash
#
# Automates starting a job session based on a selected Taskwarrior job task.
# - Selects a job task
# - Finds the repo name from the associated Logseq file
# - Starts/attaches to a tmux session
# - Opens the corresponding VS Code workspace
...
# --- 1. Select a job task and find its file ---
TASK_DESC=$("$SCRIPTS_DIR/taskwarrior/select_job_task.sh")
...
# --- 2. Find the repository name ---
REPO_NAME=$(grep "Source Repository" "$LOGSEQ_FILE" | awk -F': ' '{print $2}')

if [[ -z "$REPO_NAME" ]]; then
    echo "Error: 'Source Repository' not found in '$LOGSEQ_FILE'." >&2
    exit 1
fi
